...
def validar_formulario_cotizacion(texto_pdf: str) -> ValidacionFormulario:
    ...
    if not match_buque or not match_buque.group(1).strip():
        ...
        # Intentar separar nombre y tipo
        partes = [p.strip() for p in texto_buque.split(',') if p.strip()]
        if len(partes) >= 2:
            datos['tipo_buque'] = partes[-1]
        else:
            # Si no hay coma, intentar por palabras clave
            palabras = texto_buque.split()
            for i, palabra in enumerate(palabras):
                if any(kw in palabra.lower() for kw in 
                       [k for v in RANGO_KEYWORDS.values() for k in v]):
    ...
    # === RFC ===
    match_rfc = re.search(r'RFC de la compañía\.\s+([A-ZÑ&]{3,4}\d{6}[A-Z0-9]{2,3})', 
    ...
    if not servicio_texto or servicio_texto.lower() in ['', 'n/a', 'na', 'ninguno']:
        ...
        # Validar que no sea solo el tipo de buque (error común del OCPM)
        if any(kw in servicio_texto.lower() for kw in 
               [k for v in RANGO_KEYWORDS.values() for k in v]):
            if not any(sv in servicio_texto.lower() for sv in 
                      ['verificación', 'evaluación', 'renovación', 'inspección', 
                       'auditoría', 'certificación']):
                errores.append(f"⚠️ En SERVICIOS SOLICITADOS parece haber puesto el TIPO DE BUQUE ('{servicio_texto}') en lugar del servicio. Corregir.")
